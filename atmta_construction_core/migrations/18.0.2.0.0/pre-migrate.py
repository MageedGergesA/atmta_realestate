...
from odoo.addons.atmta_construction_core.hooks import W25_RULE_XMLIDS
...
def migrate(cr, version):
    ...
    # `construction_member_ids` itself: the field definition moved here so the
    # rules could. The relation table is named explicitly in both the old and
    # the new declaration and is untouched, so this hands over the definition
    # and not one membership row.
    cr.execute("""
        UPDATE ir_model_data SET module = %s
         WHERE module = %s AND model = 'ir.model.fields' AND name = %s
    """, (NEW, OLD, 'field_realestate_project__construction_member_ids'))
    _logger.info("Wave 25: %s field identifier re-pointed to %s",
                 cr.rowcount, NEW)
